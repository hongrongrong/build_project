...
//#define FILE_LOCK_NAME "/tmp/emaildb.fl"
#define FILE_LOCK_NAME "/home/user1/hong/smdd/demo/99-project_bak/tmp/emaildb.fl"
...
{
	if (!db || !conf) return -1;
	int fd = file_lock(FILE_LOCK_NAME);
	HSQLITE hconn = 0;
...
int add_mailtosend(const char* db, struct mailtosend* mail)
{
	...
	mail->mailto, mail->mailcc, mail->mailsub, mail->mailbody, mail->mailattach, mail->tasktime, mail->sendtime, mail->status, mail->retry);
	sqlite_execute(hconn, exec);
	sqlite_disconn(hconn);
	file_unlock(fd);
	return 0;
...
{
	if(!db || !pCnt) return -1;
	int nFd = file_lock(FILE_LOCK_NAME);
    HSQLITE hConn = 0;
