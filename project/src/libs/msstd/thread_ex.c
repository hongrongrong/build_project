...
void ms_pool_thread_init(ms_thread_pool *pool_task, int thread_num, void *(*pthread_main)(void *arg))
{
	...
	pool_task->pthreadId = (pthread_t *)malloc(thread_num * sizeof(pthread_t));
...
{
	if(!new_task){
		printf("pool add task failed!\n");
		return -1;
	}
	if(pool_task->cur_queue_size > MAX_THREAD_POOL_TASK){
		printf("pool task(%d > %d) too much!\n", pool_task->cur_queue_size, MAX_THREAD_POOL_TASK);
		return -1;
	}
